...
def draw_network(screen, step):
    ...
    if step >= 5:
        # 显示完整的 dL/dy_pred 推导过程，已向下挪动避免重叠，同时向内移动以避免超出边界:
        # 推导过程：
        # dL/dy_pred = 0.5 × 2 × (y_pred - y_target)
        #             = 0.5 × 2 × (6.5 - 7.0)
        #             = 6.5 - 7.0
        #             = -0.5
        pos_x = output_pos[0] - 20  # 向内调整 x 坐标
        ...
        screen.blit(line1, (pos_x, offset_y))
        screen.blit(line2, (pos_x, offset_y + 20))
        screen.blit(line3, (pos_x, offset_y + 40))
        screen.blit(line4, (pos_x, offset_y + 60))
