...
# 设置字体
font = pygame.font.SysFont("Consolas", 20)  # 使用等宽字体，缩小字号
...
def draw_network(screen, step):
    ...
    if step >= 6:
        # 分两行显示
        text_line1 = font.render("grad_w2 =", True, COLORS["gradients"])
        text_line2 = font.render("[-0.75, -1.25]", True, COLORS["gradients"])

        # 计算安全位置
        base_x = min(
            max((hidden1_pos[0] + output_pos[0]) // 2 - 50, 20), screen_width - 120
        )
        base_y = min(
            max((hidden1_pos[1] + output_pos[1]) // 2 - 30, 20), screen_height - 60
        )

        screen.blit(text_line1, (base_x, base_y))
        screen.blit(text_line2, (base_x, base_y + 20))
    ...
    if step >= 7:
        ...
        # 定位到隐藏层中间
        text_rect.center = (
            (hidden1_pos[0] + hidden2_pos[0]) // 2,
            (hidden1_pos[1] + hidden2_pos[1]) // 2 + 40,
        )
    ...
    if step >= 8:
        ...
        text_rect = text.get_rect()
        # 定位到输入层下方
        text_rect.midtop = (input_pos[0], input_pos[1] + 50)
        text_rect.clamp_ip(screen_rect)
        screen.blit(text, text_rect)
    ...
    if step >= 4:
        ...
        text_rect = text.get_rect(top=output_pos[1] + 40, left=output_pos[0] + 40)
        text_rect.clamp_ip(screen_rect)  # 强制限制在屏幕内
        screen.blit(text, text_rect)
