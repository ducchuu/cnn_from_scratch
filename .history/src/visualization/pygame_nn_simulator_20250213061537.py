...
def draw_network(screen, step):
    ...
    if step < 9:
        ...
        text = font.render(f"w=1.00", True, COLORS["weights"])
        screen.blit(text, hidden1_output_pos["top1"])
        text = font.render(f"b=0.00", True, COLORS["weights"])
        screen.blit(text, hidden1_output_pos["bottom1"])
        text = font.render(f"w=2.00", True, COLORS["weights"])
        screen.blit(text, hidden2_output_pos["top1"])
        text = font.render(f"b=0.00", True, COLORS["weights"])
        screen.blit(text, hidden2_output_pos["bottom1"])
    else:
        # 更新后的参数
        ...
        text = font.render(f"w=1.075", True, COLORS["updated_params"])
        screen.blit(text, hidden1_output_pos["top1"])
        text = font.render(f"b=0.05", True, COLORS["updated_params"])
        screen.blit(text, hidden1_output_pos["bottom1"])
        text = font.render(f"w=2.125", True, COLORS["updated_params"])
        screen.blit(text, hidden2_output_pos["top1"])
        text = font.render(f"b=0.05", True, COLORS["updated_params"])
        screen.blit(text, hidden2_output_pos["bottom1"])
    ...
    if step >= 6:
        # 输出层权重梯度
        text = font.render("grad_w2 =", True, COLORS["gradients"])
        screen.blit(text, hidden1_output_pos["top2"])
        text = font.render("[-0.75, -1.25]", True, COLORS["gradients"])
        screen.blit(
            text, (hidden1_output_pos["top2"][0], hidden1_output_pos["top2"][1] + 20)
        )

    if step >= 7:
        # 隐藏层梯度
        text = font.render("dL/dh =", True, COLORS["gradients"])
        screen.blit(text, hidden1_output_pos["bottom2"])
        text = font.render("[-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(
            text,
            (hidden1_output_pos["bottom2"][0], hidden1_output_pos["bottom2"][1] + 20),
        )

    if step >= 8:
        # 输入层权重梯度
        text = font.render("grad_w1 =", True, COLORS["gradients"])
        screen.blit(text, input_hidden1_pos["top2"])
        text = font.render("[-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(
            text, (input_hidden1_pos["top2"][0], input_hidden1_pos["top2"][1] + 20)
        )

    # 3. 节点值显示（使用更大的间距）
    if step >= 1:
        text = font.render(f"z={z1[0]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden1_pos[0] + 30, hidden1_pos[1] - 60))
        text = font.render(f"z={z1[1]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden2_pos[0] + 30, hidden2_pos[1] - 60))

    if step >= 2:
        text = font.render(f"h={h[0]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden1_pos[0] + 30, hidden1_pos[1] - 30))
        text = font.render(f"h={h[1]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden2_pos[0] + 30, hidden2_pos[1] - 30))
    ...
    if step >= 4:
        ...
        screen.blit(text, (output_pos[0] + 40, output_pos[1] + 60))

    if step >= 5:
        # 显示完整的 dL/dy_pred 推导过程，已向下挪动避免重叠:
        # 推导过程：
        # dL/dy_pred = 0.5 × 2 × (y_pred - y_target)
        #             = 0.5 × 2 × (6.5 - 7.0)
        #             = 6.5 - 7.0
        #             = -0.5
        line1 = font.render(
            "dL/dy_pred = 0.5 × 2 × (y_pred - y_target)", True, COLORS["gradients"]
        )
        line2 = font.render(
            "           = 0.5 × 2 × (6.5 - 7.0)", True, COLORS["gradients"]
        )
        line3 = font.render("           = 6.5 - 7.0", True, COLORS["gradients"])
        line4 = font.render("           = -0.5", True, COLORS["gradients"])
        # 调整推导过程显示位置，向下挪动80个像素
        offset_y = output_pos[1] + 80
        screen.blit(line1, (output_pos[0] + 40, offset_y))
        screen.blit(line2, (output_pos[0] + 40, offset_y + 20))
        screen.blit(line3, (output_pos[0] + 40, offset_y + 40))
        screen.blit(line4, (output_pos[0] + 40, offset_y + 60))
