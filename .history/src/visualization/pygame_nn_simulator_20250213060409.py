...
def draw_network(screen, step):
    ...
    if step >= 5:
        """
        详细推导 dL/dy_pred 的计算过程:

        1. 定义损失函数（均方误差 MSE）:
            L = 0.5 * (y_pred - y_target)^2
        ...
        4. 由于 a = y_pred - y_target，所以：
            da/dy_pred = 1

        5. 根据链式法则：
            dL/dy_pred = (dL/da) * (da/dy_pred) = a * 1 = y_pred - y_target

        因此:
            dL/dy_pred = y_pred - y_target

        例如，当 y_pred = 6.5 且 y_target = 7.0 时,
            dL/dy_pred = 6.5 - 7.0 = -0.5

        注：
            加入 0.5 系数是为了在求导时抵消平方函数导数中出现的 2，从而使梯度计算更为直接。
        """
        text = font.render("dL/dy_pred", True, COLORS["gradients"])
        screen.blit(text, (output_pos[0] + 40, output_pos[1]))
        text = font.render("= -0.5", True, COLORS["gradients"])
