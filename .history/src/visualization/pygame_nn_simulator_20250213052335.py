...
def get_edge_text_positions(start_pos, end_pos):
    ...
    # 返回更多的文本位置选项
    positions = {
        "top2": (mid_x + normal_x * 80, mid_y + normal_y * 80),  # 最上层
        "top1": (mid_x + normal_x * 40, mid_y + normal_y * 40),  # 上层
        "middle": (mid_x, mid_y),  # 中间
        "bottom1": (mid_x - normal_x * 40, mid_y - normal_y * 40),  # 下层
        "bottom2": (mid_x - normal_x * 80, mid_y - normal_y * 80),  # 最下层
    }
    return positions
...
def draw_network(screen, step):
    ...
    if step >= 6:
        # 输出层权重梯度（左右路径分开显示）
        text = font.render("grad_w2 = [-0.75", True, COLORS["gradients"])
        screen.blit(
            text,
            (hidden1_output_pos["top1"][0] - 30, hidden1_output_pos["top1"][1] - 15),
        )
        text = font.render(", -1.25]", True, COLORS["gradients"])
        screen.blit(
            text,
            (hidden2_output_pos["top2"][0] + 30, hidden2_output_pos["top2"][1] + 15),
        )
    ...
    if step >= 7:
        ...
        screen.blit(
            text,
            (hidden1_output_pos["bottom1"][0] - 40, hidden1_output_pos["bottom1"][1]),
        )
        text = font.render("= [-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(
            text,
            (hidden2_output_pos["bottom2"][0] + 40, hidden2_output_pos["bottom2"][1]),
        )

    if step >= 8:
        # 输入层权重梯度（分左右显示）
        text = font.render("grad_w1 (left)", True, COLORS["gradients"])
        screen.blit(
            text,
            (input_hidden1_pos["top1"][0] - 100, input_hidden1_pos["top1"][1] - 20),
        )
        text = font.render("= -0.5", True, COLORS["gradients"])
        screen.blit(
            text, (input_hidden1_pos["top1"][0] - 100, input_hidden1_pos["top1"][1] + 0)
        )

        text = font.render("grad_w1 (right)", True, COLORS["gradients"])
        screen.blit(
            text, (input_hidden2_pos["top2"][0] + 60, input_hidden2_pos["top2"][1] - 20)
        )
        text = font.render("= -1.0", True, COLORS["gradients"])
        screen.blit(
            text, (input_hidden2_pos["top2"][0] + 60, input_hidden2_pos["top2"][1] + 0)
        )
    ...
    if step >= 1:
        ...
        screen.blit(text, (hidden1_pos[0] + 30, hidden1_pos[1] - 70))  # 上移20像素
        text = font.render(f"z={z1[1]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden2_pos[0] + 30, hidden2_pos[1] - 70))  # 上移20像素

    if step >= 2:
        # 调整隐藏层h值的显示位置到节点下方
        text = font.render(f"h={h[0]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden1_pos[0] + 30, hidden1_pos[1] + 20))  # 下移20像素
        text = font.render(f"h={h[1]:.1f}", True, COLORS["activation"])
        screen.blit(text, (hidden2_pos[0] + 30, hidden2_pos[1] + 20))  # 下移20像素

    # 4. 输出层信息（使用更大的垂直间距）
    if step >= 3:
        text = font.render("y_pred=6.5", True, COLORS["gradients"])
        screen.blit(text, (output_pos[0] + 40, output_pos[1] - 40))  # 上移20像素

    if step >= 4:
        text = font.render("L = 0.125", True, COLORS["loss"])
        screen.blit(text, (output_pos[0] + 40, output_pos[1] + 60))  # 下移20像素

    if step >= 5:
        text = font.render("dL/dy_pred = -0.5", True, COLORS["gradients"])
        screen.blit(text, (output_pos[0] + 40, output_pos[1] + 10))  # 保持原有位置
