...
def draw_network(screen, step):
    ...
    # 2. 显示梯度信息（在上下不同层次）
    if step >= 6:
        # 输出层权重梯度（使用top1和top2错开显示）
        text = font.render("grad_w2 = [-0.75, -1.25]", True, COLORS["gradients"])
        screen.blit(text, hidden1_output_pos["top1"])  # 修改为top1
        screen.blit(text, hidden2_output_pos["top2"])  # 修改为top2

    if step >= 7:
        # 隐藏层梯度（使用不同层级错开显示）
        text = font.render("dL/dh = [-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(text, hidden1_output_pos["bottom1"])  # 修改为bottom1
        screen.blit(text, hidden2_output_pos["bottom2"])  # 修改为bottom2

    if step >= 8:
        # 输入层权重梯度（使用不同偏移量）
        text = font.render("grad_w1 = [-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(
            text, (input_hidden1_pos["top1"][0] - 20, input_hidden1_pos["top1"][1])
        )  # 增加横向偏移
        screen.blit(
            text, (input_hidden2_pos["top2"][0] + 20, input_hidden2_pos["top2"][1])
        )  # 增加横向偏移
