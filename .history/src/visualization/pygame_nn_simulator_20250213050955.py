...
def draw_network(screen, step):
    ...
    def get_edge_text_positions(start_pos, end_pos):
        ...
        # 返回多个文本位置（上中下）
        positions = {
            "top": (mid_x + normal_x * 30, mid_y + normal_y * 30),
            "middle": (mid_x, mid_y),
            "bottom": (mid_x - normal_x * 30, mid_y - normal_y * 30),
        }
        return positions
    ...
    if step >= 6:
        ...
        screen.blit(text, hidden1_output_pos["top"])

    if step >= 7:
        # 隐藏层梯度
        text = font.render("dL/dh = [-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(text, hidden1_output_pos["bottom"])
        screen.blit(text, hidden2_output_pos["bottom"])

    if step >= 8:
        # 输入层权重梯度
        text = font.render("grad_w1 = [-0.5, -1.0]", True, COLORS["gradients"])
        screen.blit(text, input_hidden1_pos["top"])
