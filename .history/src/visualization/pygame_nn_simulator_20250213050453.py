...
def draw_network(screen, step):
    ...
    # 在连线上显示权重和偏置
    def draw_weight_bias(start_pos, end_pos, w, b, color, offset=0):
        mid_x = (start_pos[0] + end_pos[0]) // 2
        mid_y = (start_pos[1] + end_pos[1]) // 2 + offset
        text = font.render(f"w={w:.2f}, b={b:.2f}", True, color)
        screen.blit(text, (mid_x - 40, mid_y - 10))

    # 绘制初始权重和偏置
    if step < 9:  # 未更新参数时显示初始值
        # 输入层到隐藏层的权重和偏置
        draw_weight_bias(input_pos, hidden1_pos, 1.0, 0.5, COLORS["weights"], -20)
        draw_weight_bias(input_pos, hidden2_pos, 2.0, 0.5, COLORS["weights"], 20)
        # 隐藏层到输出层的权重和偏置
        draw_weight_bias(hidden1_pos, output_pos, 1.0, 0.0, COLORS["weights"], -20)
        draw_weight_bias(hidden2_pos, output_pos, 2.0, 0.0, COLORS["weights"], 20)
    else:  # 参数更新后显示新值
        # 输入层到隐藏层的新权重和偏置
        draw_weight_bias(
            input_pos, hidden1_pos, 1.05, 0.55, COLORS["updated_params"], -20
        )
        draw_weight_bias(input_pos, hidden2_pos, 2.1, 0.6, COLORS["updated_params"], 20)
        # 隐藏层到输出层的新权重和偏置
        draw_weight_bias(
            hidden1_pos, output_pos, 1.075, 0.05, COLORS["updated_params"], -20
        )
        draw_weight_bias(
            hidden2_pos, output_pos, 2.125, 0.05, COLORS["updated_params"], 20
        )
    ...
    if step >= 4:
        ...
        screen.blit(text, (output_pos[0] + 40, output_pos[1] + 30))
